...
public class MainFrame extends javax.swing.JFrame implements ProgressListener, BrushListener {
    ...
    public void setRSTNSGAIIIData(int numDim, ArrayList <float[]> points){
    	STFFile f = new STFFile("RST_NSGAIII");
    	f.addProgressListener(this);
    	f.setTempNumDimensions(numDim);
    	boolean isStringLabel[] = new boolean[numDim];
    	
    	String label = "x";
    	for(int i=0; i< numDim; i++){
    		f.addStringLabel(label + (new Integer(i+1)).toString());
    		isStringLabel[i] = true;
    	}
    	f.setIsStringLabel(isStringLabel);
