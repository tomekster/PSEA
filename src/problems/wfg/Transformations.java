...
public class Transformations {
	...
	public double bPoly(double y, double alpha) {
		if (!(alpha > 0)) {
			...
			// JMetalLogger.logger.severe("wfg.Transformations.bPoly: Param
			// alpha " + "must be > 0");
		}
	...
	public double sMulti(double y, int A, int B, double C) {
		...
		tmp1 = ((double) 4.0 * A + (double) 2.0) * (double) Math.PI
				* ((double) 0.5 - Math.abs(y - C) / ((double) 2.0 * ((double) Math.floor(C - y) + C)));
		tmp2 = (double) 4.0 * B * (double) StrictMath
				.pow(Math.abs(y - C) / ((double) 2.0 * ((double) Math.floor(C - y) + C)), (double) 2.0);
	...
	public double bParam(double y, double u, double A, double B, double C) {
		...
		v = A - ((double) 1.0 - (double) 2.0 * u) * Math.abs((double) Math.floor((double) 0.5 - u) + A);
		exp = B + (C - B) * v;
